...
def find_business_hours() :
    ...
    # 부모요소 발견되면 자식요소 찾기(영업종료 / 영업 중 - 운영시간)
...
while(loop):
    ...
    # 다음 페이지가 disabled 속성인지 [true / false] 페이지 넘길때마다 계속 확인해줘야 함(페이지 새로 로드 시, 버튼 상태 값이 바뀜)
    ...
    # 메모리 부족 이슈 때문에 저장을 분리합니다.
    # 3페이지가 되면 드라이버를 끄고 다시 3페이지부터 시작합니다.
    if page_no == '3':
        # 드라이버 끄고
        driver.quit()
        # 드라이버 초기화
        ...
        # 3 페이지를 클릭합니다.
        driver.find_element(By.XPATH, '//*[@id="app-root"]/div/div[3]/div[2]/a[4]').click()
        print("3 페이지를 진행합니다.")
        page_no = driver.find_element(By.XPATH, '//a[contains(@class, "mBN2s qxokY")]').text
    ...
    for index, e in enumerate(restaurant_elements, start=1):
        ...
        try:
            ...
            #     switch_right()
            #     is_menu = WebDriverWait(driver, 2).until(
            #         EC.element_to_be_clickable((By.XPATH, '//div[@class="flicking-camera"]/a/span[text()="메뉴"]'))
            #     )
            #
            # except Exception as e:
