...
# 네이버 지도 로드
URL = 'https://map.naver.com/search/암사동 맛집'
driver.get(URL)
...
while (loop):
    ...
    for index, e in enumerate(restaurant_elements, start=1):
        ...
        # 가게 고유 아이디
        try:
            store_id = driver.find_element(By.XPATH,
                                           '//div[@class="flicking-camera"]/a').get_attribute('href').split('/')[4]
        except Exception as e:
    ...
    if stores:
        ...
        df.to_csv('강동구/암사동 맛집2.csv', mode='a', encoding='utf-8-sig', index=False, header=is_first_save)
